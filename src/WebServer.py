...
class WebServer:
    ...
    def serialize_outcom_msg(
        ...
        if msg_type == INSERT_MSG:
            ...
            tmp_handler[PUT_MSG][MSG_STRUCT][DATA].pop(CONTENT) # remove 'content' key and add 'status'
            tmp_handler[PUT_MSG][MSG_STRUCT][DATA][STATUS] = str(extra_info[1]) + " " + str(extra_info[2])
            msg_type = PUT_MSG
    ...
    def notify_server(
        ...
        while True:
            ...
            if not send_data(dest_host, msg.encode(ENC_DATA_FORMAT)):
                ...
                if self.SERVERS_LIST.get_size() == 1:
                    utils.log.WARN("One server only in the list")
                    return
    ...
    def handle_get_request(self, conn_socket=None, conn_host=None, recv_json=None):
        ...
        if self.DB.__contains__(item):
            ...
            response = self.serialize_outcom_msg(msg_type=RESPONSE_GET_MSG, extra_info=(item, content))
    ...
    def handle_put_request(self, conn_socket=None, conn_host=None, recv_json=None):
        ...
        if self.SERVERS_LIST.get_size() == 1:
            response = self.serialize_outcom_msg(msg_type=RESPONSE_PUT_MSG, extra_info=(item, 200, OK_MSG))
    ...
    def handle_fwrd_get_request(self, conn_socket=None, conn_host=None, recv_json=None):
        ...
        if self.is_creator(creator_host):
            ...
            if cont is NOT_FOUND_MSG:
                ...
                response = self.serialize_outcom_msg(msg_type=RESPONSE_GET_MSG, extra_info=(item, cont))
...
def main(arguments):
    ...
    parser.add_argument(
        "-i", "--ip", help="Previous (in list) IP address", type=str, required=False
    )
    parser.add_argument(
        "-p", "--port", help="Previous (in list) port number", type=int, required=False
    )
    parser.add_argument("-v", "--verbose", action="store_true")

    args = parser.parse_args(arguments)

    if args.ip and not utils.valid_ip(args.ip):
